...
const Bookings = () => {
    ...
    try {
        ...
        if (data.length > 0 && data[0].passengers) {
            const flightNo = data[0].flightNo; // ✅ extract flight number
            const enrichedPassengers = data[0].passengers.map(p => ({
                ...p,
                flightNo // ✅ assign flight number to each passenger
            }));
    ...
    const generateBoardingPass = async (passenger) => {
        ...
        try {
            ...
            console.log("🛫 Flight No:", passenger.flightNo);
            ...
            if (!rawText) {
                throw new Error("Flight API returned empty response. Check flight number.");
            }
            ...
            autoTable(doc, {
                ...
                body: [
                    ...
                    ["From -> To", `${flightData.source} -> ${flightData.destination}`],
                    ["Departure", flightData.date],
